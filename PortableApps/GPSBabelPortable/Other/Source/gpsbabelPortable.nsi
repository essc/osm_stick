...
!define PORTABLEAPPNAME "GPSBabel Portable"
!define NAME "GPSBabelPortable"
!define APPNAME "GPSBabelPortable"
!define VER "0.1.0222.13620"
!define WEBSITE ""
!define DEFAULTEXE "gpsbabelGUI.exe"
!define DEFAULTAPPDIR "gpsbabel"
...
Section "Main"
	...
	RegistryBackup:
		...
		;=== Backup the registry
		${registry::KeyExists} "HKEY_CURRENT_USER\Software\GPSBabel-BackupBy-GPSBabelPortable" $R0
		StrCmp $R0 "0" RestoreTheKey
		${registry::KeyExists} "HKEY_CURRENT_USER\Software\GPSBabel" $R0
		StrCmp $R0 "-1" RestoreTheKey
		${registry::MoveKey} "HKEY_CURRENT_USER\Software\GPSBabel" "HKEY_CURRENT_USER\Software\GPSBabel-BackupBy-GPSBabelPortable" $R0
		Sleep 100

	RestoreTheKey:
		IfFileExists "$SETTINGSDIRECTORY\GPSBabelPortable.reg" "" LaunchNow

		IfFileExists "$WINDIR\system32\reg.exe" "" RestoreTheKey9x
			nsExec::ExecToStack `"$WINDIR\system32\reg.exe" import "$SETTINGSDIRECTORY\GPSBabelPortable.reg"`
			Pop $R0
			StrCmp $R0 '0' LaunchNow ;successfully restored key

	RestoreTheKey9x:
		${registry::RestoreKey} "$SETTINGSDIRECTORY\GPSBabelPortable.reg" $R0
		StrCmp $R0 '0' LaunchNow ;successfully restored key
	...
	CheckRunning:
		...
		${registry::SaveKey} "HKEY_CURRENT_USER\Software\GPSBabel" "$SETTINGSDIRECTORY\GPSBabelPortable.reg" "" $0
		Sleep 100

	SetOriginalKeyBack:
		${registry::DeleteKey} "HKEY_CURRENT_USER\Software\GPSBabel" $R0
		Sleep 100
		${registry::KeyExists} "HKEY_CURRENT_USER\Software\GPSBabel-BackupBy-GPSBabelPortable" $R0
		StrCmp $R0 "-1" TheEnd
		${registry::MoveKey} "HKEY_CURRENT_USER\Software\GPSBabel-BackupBy-GPSBabelPortable" "HKEY_CURRENT_USER\Software\GPSBabel" $R0
		Sleep 100
		Goto TheEnd
